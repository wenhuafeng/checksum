...
#define CRC_TAB_LEN 256
#define ONE_BYTE_LEN 8

struct Crc32DataType {
    uint32_t crc32;
};
...
static void CRC32_Init(struct Crc32DataType *crc)
{
    crc->crc32 = CRC_INIT;
}

static void CRC32_Update(struct Crc32DataType *crc, const uint8_t *addr, size_t len)
{
    while (len != 0x00) {
        len--;
        crc->crc32 = g_crcTable[(crc->crc32 ^ *addr++) & 0xFFL] ^
                     (crc->crc32 >> ONE_BYTE_LEN);
    }
}

static uint32_t CRC32_Value(struct Crc32DataType *crc)
{
...
{
    uint32_t ret;
    struct Crc32DataType crc;

    CRC32_Init(&crc);
    CRC32_Update(&crc, start, length);
    ret = CRC32_Value(&crc);
...
#if 0
#define THREE_BYTE_LEN 24
#define TWO_BYTE_LEN 16
#define ONE_BYTE_LEN 8
#define PAD_WITH_ZEROES_LEN 4
...
// initialize the members of the allocated Crc32DataType struct
static void Crc32Init(Crc32DataType *crc32Config)
{
...
static void Crc32Update(Crc32DataType *crc32Config, const uint8_t *src, uint32_t len)
{
    uint8_t c;
    uint32_t crc;
    uint32_t length = len;

    crc = crc32Config->currentCrc;
    crc32Config->byteCountCrc += length;

    while (length != 0x00) {
        length--;
        c = *src & 0xff;
        src++;
        crc = (crc << ONE_BYTE_LEN) ^ g_crc32Table[(crc >> THREE_BYTE_LEN) ^ c];
    }
...
static void Crc32Finalize(Crc32DataType *crc32Config, uint32_t *hash)
{
    uint32_t i;
    ...
    // pad with zeroes
    if (byteCount % PAD_WITH_ZEROES_LEN != 0x00) {
        for (i = byteCount % PAD_WITH_ZEROES_LEN; i < PAD_WITH_ZEROES_LEN; i++) {
            crc = (crc << ONE_BYTE_LEN) ^ g_crc32Table[(crc >> THREE_BYTE_LEN) ^ 0];
        }
    }
...
uint32_t CalculateCrc32(const uint8_t *start, size_t length)
{
    uint32_t crc32;
    Crc32DataType crcInfo;
    //const struct BcaType *config = NULL; // Just used for calculate the offset, must not change the region it points to.
    //uint32_t bypassStartAddress = 0;
    //uint32_t bypassEndAddress = length;

    // Initialize the CRC32 information
    Crc32Init(&crcInfo);

    //if (length <= bypassStartAddress) {
        Crc32Update(&crcInfo, (uint8_t *)start, length);
    //} else {
    //    // Assume that crcExpectedValue address (4 byte) resides in crc addresses completely
    //    Crc32Update(&crcInfo, (uint8_t *)start, bypassStartAddress);
    //    Crc32Update(&crcInfo, (uint8_t *)(start + bypassEndAddress), length - bypassEndAddress);
    //}

    // Finalize the CRC calculations
    Crc32Finalize(&crcInfo, &crc32);
